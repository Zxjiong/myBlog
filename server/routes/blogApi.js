...
router.get("/getBadWord", function (req, res, next) {
  BadWords.find(function (err, data) {
    if (err) {
      res.json({
        status: '1',
        msg: err.message,
  ...
  if (req.cookies && req.cookies.userId) {
    ...
    Users.findOne({ userId: userId }, function (err, userDoc) {
      if (err) {
        ...
        if (userDoc) {
          for (var i = 0; i < userDoc.blogList.length; i++) {
            if (userDoc.blogList[i].blogId == blogId) {
              for (var j = 0; j < userDoc.blogList[i].commentList.length; j++) {
                if(userDoc.blogList[i].commentList[j].commentId == commentId){
                  userDoc.blogList[i].commentList.splice(j, 1);
                  userDoc.blogList[i].commentNum = userDoc.blogList[i].commentNum -1;
                }
              }
            }
          }
          userDoc.save(function (err1, doc) {
            if (err1) {
              ...
              if (doc) {
                Blogs.findOne({ blogId: blogId },function(err,doc2){
                  var commentNum = doc2.commentNum;
                  let update={ commentNum:commentNum-1};
                  Blogs.findOneAndUpdate({ blogId: blogId }, {$set:update},function (err2, doc1) {
                    if (err2) {
                      res.json({
                        status: '1',
                        msg: err.message,
        ...
        if (userDoc) {
          ...
          userDoc.save(function (err1, doc) {
            if (err1) {
              ...
              if (doc) {
                Blogs.findOneAndDelete({ blogId: blogId }, function (err2, doc1) {
                  if (err2) {
                    res.json({
                      status: '1',
                      msg: err.message,
    ...
    Users.findOne({ userId: userId }, function (err, userDoc) {
      if (err) {
        ...
        if (userDoc) {
          for (var i = 0; i < userDoc.blogList.length; i++) {
            if (userDoc.blogList[i].blogId == blogId) {
              userDoc.blogList[i].title = title;
              userDoc.blogList[i].content = content;
              userDoc.blogList[i].tags = tags;
            }
          }
          userDoc.save(function (err1, doc) {
            if (err1) {
              ...
              if (doc) {
                let update={ title:title , content:content , tags:tags}
                Blogs.findOneAndUpdate({ blogId: blogId }, {$set:update},function (err2, doc1) {
                  if (err2) {
                    res.json({
                      status: '1',
